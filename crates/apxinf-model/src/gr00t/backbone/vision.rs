...
#[cfg(feature = "cuda")]
use super::super::backend::{kernels, CudaBackend, DeviceBuffer};
...
fn add_bias(b: &dyn Backend, input: &Tensor, bias: &Tensor) -> Result<Tensor> {
    ...
    if let Some(cuda) = b.as_any().downcast_ref::<CudaBackend>() {
        return kernels::elementwise::bias_bf16(cuda.context(), input, Some(bias));
    }
    b.add_bias(input, bias)
}
...
fn layer_norm(
    ...
    #[cfg(feature = "cuda")]
    if let Some(cuda) = b.as_any().downcast_ref::<CudaBackend>() {
        return kernels::norm::layer_bf16(cuda.context(), input, weight, bias, eps);
    }
    b.layer_norm(input, weight, bias, eps)
}
...
fn vision_attention(
    ...
    #[cfg(feature = "cuda")]
    if let Some(cuda) = b.as_any().downcast_ref::<CudaBackend>() {
        return kernels::attention::noncausal(cuda.context(), query, key, value, n_heads, head_dim);
    }
    b.vision_sdpa(query, key, value, sequence_len, n_heads, head_dim)
}
...
fn forward_impl(
    ...
    .map_err(|error| Error::Other(format!("vision patch embedding matmul failed: {error}")))?;
    x = add_bias(b, &x, &w.patch_embed_bias)
        .map_err(|error| Error::Other(format!("vision patch embedding bias failed: {error}")))?;
    ...
    x = b
        .add(&x, &pos_embeds)
        .map_err(|error| Error::Other(format!("vision position embedding add failed: {error}")))?;
    ...
    for (i, blk) in w.blocks.iter().enumerate() {
        ...
        let qkv = if let Some(norm_matmul) = norm_matmul {
            ...
            let normed = layer_norm(b, &x, &blk.norm1_w, &blk.norm1_b, eps).map_err(|error| {
                Error::Other(format!(
                    "vision block {i} pre-attention norm failed: {error}"
                ))
            })?;
            vision_matmul(matmul, b, &qkv_name, &normed, &blk.qkv_w).map_err(|error| {
                Error::Other(format!("vision block {i} QKV matmul failed: {error}"))
            })?
        };
...
fn merge_primary(
    ...
    let normed = layer_norm(b, x, &m.norm_w, &m.norm_b, eps)?;
...
fn merge_deepstack(
    ...
    let normed = layer_norm(b, &merged, &m.norm_w, &m.norm_b, eps)?;
    let h = vision_matmul(
        matmul,
        b,
...
fn apply_bias_gelu(b: &dyn Backend, input: &Tensor, bias: &Tensor) -> Result<Tensor> {
    #[cfg(feature = "cuda")]
    if let Some(cuda) = b.as_any().downcast_ref::<CudaBackend>() {
        return kernels::activation::bias_gelu_bf16(cuda.context(), input, Some(bias));
    }

    let biased = add_bias(b, input, bias)?;
    b.gelu_tanh(&biased)
}
...
fn apply_bias_residual(
    ...
    #[cfg(feature = "cuda")]
    if let Some(cuda) = b.as_any().downcast_ref::<CudaBackend>() {
        return kernels::fused::bias_residual_bf16(
            cuda.context(),
            projection,
            Some(bias),
            residual,
    ...
    let projection = add_bias(b, projection, bias)?;
...
fn apply_bias_residual_layer_norm(
    ...
    let projection = add_bias(b, projection, projection_bias)?;
    let hidden = b.add(residual, &projection)?;
    let normalized = layer_norm(b, &hidden, norm_weight, norm_bias, eps)?;
...
fn split_vision_qkv(
    ...
    #[cfg(feature = "cuda")]
    if let Some(cuda) = b.as_any().downcast_ref::<CudaBackend>() {
        let split = kernels::attention::split_qkv_bias_bf16(
            cuda.context(),
            qkv,
            Some(bias),
            n_heads,
...
fn apply_vision_rope_pair(
    ...
    if let PreparedVisionPositionBuffer::Cuda(positions) = prepared {
        ...
        return kernels::rope::apply_vision_2d_pair(
            cuda.context(),
            q,
            k,
            n_heads,
...
fn segmented_vision_sdpa(
    ...
    if let Some(cuda) = b.as_any().downcast_ref::<CudaBackend>() {
        ...
        for &row_count in segment_lengths {
            ...
            let q_segment =
                kernels::elementwise::contiguous_rows(cuda.context(), &q, first_row, row_count)?
                    .reshape(shape.clone())?;
            let k_segment =
                kernels::elementwise::contiguous_rows(cuda.context(), &k, first_row, row_count)?
                    .reshape(shape.clone())?;
            let v_segment =
                kernels::elementwise::contiguous_rows(cuda.context(), &v, first_row, row_count)?
                    .reshape(shape)?;
            segments.push(vision_attention(
                b, &q_segment, &k_segment, &v_segment, row_count, n_heads, head_dim,
            )?);
            first_row = first_row
        ...
        for segment in segments {
            output = kernels::elementwise::concat_rows_bf16(cuda.context(), &output, &segment)?;
        }
        return Ok(output);
    }
